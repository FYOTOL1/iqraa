...
import { PatchSuggestion } from "../../../redux/reducers/SuggestionsSlice";

export default function Card({ name, date, p, dispatch, e }) {
  const Accept = () => {
    dispatch(PatchSuggestion({ id: e.id, status: 3 }));
  };

  const unAccept = () => {
    dispatch(PatchSuggestion({ id: e.id, status: 1 }));
  };
